...
async function GetFollowingData(username: string, page: number): Promise<FollowingData[] | null> {
  try {
    const response = await fetch(
      `https://api.github.com/users/${username}/following?page=${page}&per_page=100`,
      {
        headers: {
    ...
    if (!response.ok) {
      console.error(
        `[GetFollowingData] Erro HTTP ${response.status} ao buscar following de ${username}`,
      );
      return null;
    }
    ...
    console.error(
      '[GetFollowingData] Erro ao buscar following:',
