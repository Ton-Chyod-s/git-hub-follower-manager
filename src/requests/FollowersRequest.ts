...
async function GetFollowersData(username: string, page: number): Promise<FollowersData[] | null> {
    try {
        ...
        if (!response.ok) {
            console.error(`[GetFollowersData] Erro HTTP ${response.status} ao buscar seguidores de ${username}`);
            return null;
        }
        ...
        console.error("[GetFollowersData] Erro ao buscar seguidores:", error instanceof Error ? error.message : error);
        return null;
    }
}
